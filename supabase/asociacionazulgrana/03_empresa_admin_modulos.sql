...
-- =============================================================================
-- Crea, SOLO dentro de asociacionazulgranaerp (+ el usuario en auth, que es de
-- Supabase y es compartido por diseño):
--
--   · empresa   Asociación Hernandarias Azulgrana
--               id = 498add65-8616-412a-9ee7-d5d60adba136
--   · login     admin@asociacionhernandariasazulgrana.com   rol `admin`
--   · empresa_modulos con EXACTAMENTE los 6 módulos pedidos:
...
DECLARE
  ...
  v_nombre     text := 'Asociación Hernandarias Azulgrana';
  v_email      text := 'admin@asociacionhernandariasazulgrana.com';
  v_password   text := 'CambiarEsto123!';   -- <<<<<< CAMBIAR ANTES DE EJECUTAR
  ---------------------------------------------------------------------------
  -- Los 6 módulos pedidos, con los slugs que el CÓDIGO realmente evalúa
  -- (src/components/layout/Sidebar.tsx + src/lib/modulos/route-slug-map.ts).
  v_slugs text[] := ARRAY[
  ...
  -- slugs existen en la app pero podrían no haberse insertado en ese schema
  -- (las migraciones que los crean apuntaban a `neura` / `zentra_erp`). Los
  -- damos de alta acá si faltan, con el slug exacto que evalúa el código.
...
BEGIN
  ...
  -- (= asociacionazulgranaerp) y no intenta resolver un schema tenant aparte.
