...
-- Origen: instemaq · Destino: asociacionazulgranaerp
-- =============================================================================
...
FROM (
  SELECT CASE c.relkind WHEN 'r' THEN 'tablas' WHEN 'v' THEN 'vistas'
         ...
         count(*) FILTER (WHERE n.nspname = 'asociacionazulgranaerp') AS destino
  FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
  WHERE n.nspname IN ('instemaq','asociacionazulgranaerp') AND c.relkind IN ('r','v','m','S','i')
  GROUP BY 1
) t
UNION ALL
SELECT 'objetos', 'funciones',
       count(*) FILTER (WHERE n.nspname = 'instemaq'),
       count(*) FILTER (WHERE n.nspname = 'asociacionazulgranaerp'),
       count(*) FILTER (WHERE n.nspname = 'instemaq') = count(*) FILTER (WHERE n.nspname = 'asociacionazulgranaerp')
FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname IN ('instemaq','asociacionazulgranaerp')
UNION ALL
...
WHERE n.nspname IN ('instemaq','asociacionazulgranaerp') AND NOT tg.tgisinternal
UNION ALL
...
WHERE n.nspname = 'asociacionazulgranaerp' AND co.contype = 'f'
  AND rn.nspname NOT IN ('asociacionazulgranaerp', 'auth')
UNION ALL
SELECT 'fuga_funcion', p.proname::text, 'cuerpo o search_path referencia instemaq'
FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
WHERE n.nspname = 'asociacionazulgranaerp' AND p.prokind IN ('f','p')
...
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE n.nspname = 'asociacionazulgranaerp' AND c.relkind IN ('v','m')
...
FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace
WHERE n.nspname = 'asociacionazulgranaerp' AND c.relkind = 'r'
ORDER BY 2 DESC, 1;
